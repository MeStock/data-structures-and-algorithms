...
const wordsToCharList = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    let takeFirstElement = convertToArray.shift();
    result.push(takeFirstElement);
  }
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  let difference = str.length - numberOfCharacters;
  
  str = str.slice(0, difference);
  return str;
};
...
const totalSumCSV = (str) => {
  ...
  convertToArray.forEach(val => {
    let convertToNum = Number(val);
    total = total + convertToNum;
  });
  return total;
};
...
const removeVowels = (str) => {
  ...
  let includedVowels = str.match(allVowels);
  if(includedVowels){
    for(let i = 0; i < includedVowels.length; i++){
      str = str.split(includedVowels[i]);
      str = str.join('');
    }
  }
  return str;
...
const extractVowels = (str) => {
  ...
  let sortedVowels = includedVowels.sort((a,b) => {
    if(a > b) return 1;
    if(a < b) return -1;
    return 0;
  });

  sortedVowels = sortedVowels.join('');
