...
const filterStringsWithVowels = (arr) => {
  ...
  return arr.filter(word => {
    if(word.match(vowel)) return word;
  });
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(val => {
    if(forbiddenValues.includes(val) === false) return val;
  });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(statsObj => {
    if(statsObj.baseStat > minBaseStat){
      return statsObj;
    } 
  });
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  arr.filter(statsObj => {
    if(statsObj.baseStat > minBaseStat){
      result.push(statsObj.stat.name);
    } 
  });
  return result;
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(character => {
    if(character.children === undefined) return character;
  });
};
...
const evenOddNumericValues = (arr) => {
  let char = /[a-z]/gi;
  let filteredResult = arr.filter(val => {
    if(val.toString().match(char) || typeof val === 'string'){
      return false;
    }return true;
  });
